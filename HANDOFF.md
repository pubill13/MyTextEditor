...
- 검증: Release 빌드 경고/오류 0, Core 37/37, 매크로 저장·창 수명·다단계 단일 Undo/Redo 통합 테스트 통과. 기존 Diff/도움말/설정 회귀 통과. 30MB 로딩 3회 중앙값 0.250초, 리소스 해제 최대 0.067초. 5만 줄 Diff 53ms. 이번 실행에서 모든 DPI의 수동 시각 검증은 수행하지 않았다.
- 알려진 제한: 현재 문서만 실행하며 수백 MB 매크로의 무지연 성능은 보장하지 않는다. 단계 미리보기는 처음 20,000자만 표시하고 선택 단계까지 재계산한다. 매크로 파일 손상 시 기존 파일을 덮어쓰지 않으며 사용자가 복구해야 한다.
- 다음 우선순위: 사용자 실사용 피드백, 100/125/150% 배율 시각 점검, 대용량 매크로 메모리 측정. 신규 기능보다 발견된 정확성·사용성 문제를 우선한다.

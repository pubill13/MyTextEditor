...
`1.3.1`은 변환 미리보기를 한 번 적용한 뒤 다음 미리보기에서 `변경 적용` 버튼이 사라지던 문제와 특정 문장 포함 줄 삭제의 행 경계 처리를 수정한 패치 릴리스다.

- 모든 변환 미리보기는 열릴 때 `변경 적용` 버튼을 표시하며 변경이 있을 때만 활성화한다. 적용 완료, 변경 없음, 원문 수정으로 결과가 만료된 경우에는 버튼을 숨기지 않고 비활성화한다.
- 적용 직전 미리보기의 문서 추적을 해제해 Scintilla revision 이벤트가 정상 적용을 만료 상태로 덮어쓰지 않는다. 전체 교체는 기존처럼 Undo 한 번으로 복원된다.

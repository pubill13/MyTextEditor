...
{
    private readonly HashSet<DiffWindow> _diffWindows = [];

    private void OpenDiff_Click(object sender, RoutedEventArgs e)
    {
        var dialog = new DiffSourceDialog(Documents, CurrentDocument, CurrentEditor, GetDiffOptions())
        {
            Owner = this
        };
        if (dialog.ShowDialog() == true && dialog.LeftEndpoint is not null && dialog.RightEndpoint is not null)
            ShowDiffWindow(dialog.LeftEndpoint, dialog.RightEndpoint);
    }
    ...
    private void CompareClipboard_Click(object sender, RoutedEventArgs e)
    {
        ...
        ShowDiffWindow(CreateDocumentEndpoint(CurrentDocument), new DiffEndpoint
        {
    ...
    private void CompareSelectionClipboard_Click(object sender, RoutedEventArgs e)
    {
        ...
        ShowDiffWindow(new DiffEndpoint
        {
    ...
    private void ShowDiffWindow(DiffEndpoint left, DiffEndpoint right)
    {
        var window = new DiffWindow(left, right, GetDiffOptions(), CreateDiffCallbacks(), GetDiffAppearance())
        {
            Owner = this
        };
        _diffWindows.Add(window);
        window.Closed += (_, _) => _diffWindows.Remove(window);
        window.Show();
    }
    ...
    private DiffWindowCallbacks CreateDiffCallbacks() => new()
    {
        ...
        OpenChildWindow = ShowDiffWindow,
    ...
    private DiffSourceSnapshot? GetDiffSourceSnapshot(Guid id)
    {
        ...
        return document is null ? null : new DiffSourceSnapshot(
    ...
    private void ApplyDiffAppearanceToWindows()
    {
        var appearance = GetDiffAppearance();
        foreach (var window in _diffWindows.ToArray())
            window.ApplyAppearance(appearance);
    }

    private void RefreshDiffSourceStates()
    {
        foreach (var window in _diffWindows.ToArray())
    ...
    private async Task<bool> ConfirmCloseDiffWindowsAsync()
    {
        foreach (var window in _diffWindows.ToArray())
        {
            if (await window.RequestCloseAsync()) continue;
    ...
    private void ResetDiffCloseApprovals()
    {
        foreach (var window in _diffWindows.ToArray())
            window.CancelPreparedClose();
    }
}
